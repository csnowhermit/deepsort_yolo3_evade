...
def evade_vote(tracks, other_classes, other_boxs, other_scores, height):
    ...
    real_gate_light_area_list = [other_box for (other_cls, other_box) in zip(other_classes, other_bboxes) if other_cls in ['redLight', 'greenLight', 'yellowLight']]

    # 1.3、计算真实闸机位置与检测到的闸机位置的iou
    # calc_iou(gate_area_list, real_gate_area_list)
    gate_status_list = getGateStatusList(real_gate_area_list)
    print(gate_status_list)
    ...
    for (track, which_gate, pass_status, gate_status) in zip(tracks, which_gateList, pass_status_list, gate_status_list):
        trackContent = TrackContent(gate_num=which_gate,
                                    ...
                                    gate_status=gate_status,    # 闸机门状态
                                    gate_light_status='')    # 闸机灯状态
...
def getGateStatusList(real_gate_area_list):
    ...
    if real_gate_area_list is None or len(real_gate_area_list) < 1:    # 如果没检测到闸机，说明所有闸机都open
        ...
        iou_result = calc_iou(bbox1=real_gate_area_list, bbox2=gate_area_list)
        print("iou_result:", iou_result)
        for iou in iou_result:
            # print("iou.argmax:", type(iou), iou.argmax())    # jiance
